...
class Urlizer
{
    ...
    private static function postProcessText(string $text, string $separator): string
    {
        ...
        $text = strtolower((string) preg_replace('/[^A-Za-z0-9\/]+/', $separator,
            (string) preg_replace('/([a-z\d])([A-Z])/', '\1_\2',
                (string) preg_replace('/([A-Z]+)([A-Z][a-z])/', '\1_\2',
                    (string) preg_replace('/::/', '/', (string) $text)))));
